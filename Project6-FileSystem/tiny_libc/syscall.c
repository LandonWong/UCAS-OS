...
#include <stdint.h>
#include <mailbox.h>
void sys_links(char *name1, char *name2)
{
    invoke_syscall(SYSCALL_LINKS, name1, name2, IGNORE, IGNORE);
}
void sys_linkh(char *name1, char *name2)
{
    invoke_syscall(SYSCALL_LINKH, name1, name2, IGNORE, IGNORE);
}
int sys_fopen(char *name, int access)
{
    return invoke_syscall(SYSCALL_OPEN, name, access, IGNORE, IGNORE);
}
...
uint64_t sys_cat(char *name)
{
    return invoke_syscall(SYSCALL_CAT, name, IGNORE, IGNORE, IGNORE);
}
uint64_t sys_cgdir(char *name)
{
    return invoke_syscall(SYSCALL_CGDIR, name, IGNORE, IGNORE, IGNORE);
}
void sys_touch(char *name)
{
    invoke_syscall(SYSCALL_TOUCH, name, IGNORE, IGNORE, IGNORE);
}
void sys_read_dir(char *name)
{
    invoke_syscall(SYSCALL_READDIR, name, IGNORE, IGNORE, IGNORE);
}
...
{
    invoke_syscall(SYSCALL_MKDIR, name, IGNORE, IGNORE, IGNORE);
}
void sys_mkfs()
{
...
pid_t sys_run(const char *name, int argc, char* argv[], int length)
{
    return invoke_syscall(SYSCALL_RUN, (uintptr_t)name, argc, (uintptr_t)argv, length);
}
void sys_show_exec()
{
...
pid_t sys_spawn(uintptr_t entry_point, int argc, char *argv[], spawn_mode_t mode){
    return invoke_syscall(SYSCALL_SPAWN, entry_point, argc, (uintptr_t)argv, mode);
}
...
{
    return invoke_syscall(SYSCALL_PRIORITY, pid,priority,IGNORE,IGNORE);
}
