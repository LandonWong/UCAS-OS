...
mailbox_t kernel_mbox_open(char *name)
{
    ...
    for (i = 0;i < MAX_NUM; i++)
    {
        if((mbox_list[i] != NULL) && (!kstrcmp(name, mbox_list[i]->name))){
            mbox_list[i]->cited++;
    ...
    kstrcpy(mbox_list[new->id]->name, name);
...
void kernel_mbox_send(mailbox_t mailbox, void *msg, int msg_length)
{
    ...
    {
        kmemcpy((char *)(box->buffer + box->tail), (char *)msg, MSG_MAX_SIZE - box->tail);
        box->tail = msg_length - (MSG_MAX_SIZE - box->tail);
        kmemcpy((char *)box->buffer, (char *)(msg + msg_length - box->tail), box->tail);
    }
    else
    {
        kmemcpy((char *)(box->buffer + box->tail), (char *)msg, msg_length);
        box->tail += msg_length;
    }
...
void kernel_mbox_recv(mailbox_t mailbox, void *msg, int msg_length)
{
    ...
    {
        kmemcpy((char *)msg, (char *)(box->buffer + box->head), MSG_MAX_SIZE - box->head);
        box->head = msg_length - (MSG_MAX_SIZE - box->head);
        kmemcpy((char *)(msg + msg_length - box->head), (char *)box->buffer, box->head);
    }
    else
    {
        kmemcpy((char *)msg, (char *)(box->buffer + box->head), msg_length);
        box->head += msg_length;
    }
    box->used_size -= msg_length;
